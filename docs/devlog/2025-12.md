...
### 5. Timeline Overlap Guard & Long-Press Block Move

- Enforced a non-overlapping constraint on the timeline to prevent schedules from conflicting
- Rejected time changes that would cause overlaps and provided immediate feedback via a snackbar warning
- Added a long-press drag interaction to allow moving time blocks directly to a desired time slot
- Validated all time changes at drag commit time, clearly separating gesture handling from result validation
